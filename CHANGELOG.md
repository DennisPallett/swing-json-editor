# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](http://keepachangelog.com/en/1.0.0/)
and this project adheres to [Semantic Versioning](http://semver.org/spec/v2.0.0.html).

## [1.3.0] UNRELEASED
### Added
- [11](https://github.com/DennisPallett/swing-json-editor/issues/11) Add the ability to increase/decrease font size

### Fixed

## [1.2.0] 2026-04-22
### Added
- [19](https://github.com/DennisPallett/swing-json-editor/issues/19) Added tree toolbar + add tree sorting

### Fixed
- Auto-converting content when changing formats is working again

## [1.1.0] 2026-03-23
### Added
- [24](https://github.com/DennisPallett/swing-json-editor/issues/24) Added find/replace/goto line functionality
- Tree state is restored after editing JSON/YAML

### Changed
- [28](https://github.com/DennisPallett/swing-json-editor/issues/28) Improved tab titles

### Fixed
- [27](https://github.com/DennisPallett/swing-json-editor/issues/27) Improved save/save-as menu item
- Auto-detecting formats is improved

## [1.0.0] 2026-03-18
### Added
- Initial release